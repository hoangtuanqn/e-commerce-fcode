...
void view_out_of_stock_product() {
    ...
    // Store pointers to seller's out of stock products
    for(int i = 0; i < counter_product_all; ++i) {
        if(
            strcmp(product_data[i].username, current_user.username) == 0 && 
            product_data[i].status == 2) {  // Check if product is out of stock
                
            product_ptrs[product_count++] = &product_data[i];
        }
    }

    // Sort using pointers (by sold quantity)
    ...
    if(product_count == 0) {
        msg_error("You don't have any out of stock products!\n\n");
    } else {
        for(int i = 0; i < product_count; ++i) {
            ...
            printf("\tQuantity: \033[31m%d\033[0m\n", product_ptrs[i]->quantity);
